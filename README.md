# KNX Bus Monitor & Explorer Suite
...
3. **`webinterface.py`**: A web server that starts the `knx-lens.py` and makes its interface accessible in any modern web browser.
...
*Figure: The interactive explorer in action*
...
1. **Clone or download the repository**:
   Ensure that you have the files `knx-lens-logger.py`, `knx-lens.py`, and `webinterface.py` in one directory.
...
1. **Ensure the `webinterface.py` script contains the correct command:**


from textual_serve.server import Server

Ensures that the explorer is started
server = Server("python knx-lens.py")
server.serve()
...
3. **Open your browser** and go to the displayed address (usually `http://127.0.0.1:8080`).

The operation in the browser is identical to that in the terminal.
...
* **Further** Issues: Take a look at the `knx_app_debug.log` file created by `knx-lens-logger.py`. It contains detailed information about the
